  ...
  return (
    ...
    >
      <div className="relative h-48 bg-gradient-to-br from-zinc-600 to-zinc-800">
        {(item as any).projectImage?.url ? (
          <ResponsiveImage
            src={(item as any).projectImage.url}
            alt={(item as any).projectImage.alt || item.title}
            fill
            className="object-cover group-hover:scale-105 transition-transform duration-300"
            variations={(item as any).projectImage.variations}
            targetWidth={400}
          />
        ) : (
      ...
      <div className="p-6">
          {(item as any).projectUrl && (
            <p className="text-sm text-zinc-700 font-medium mb-2">{(item as any).projectUrl}</p>
          )}
