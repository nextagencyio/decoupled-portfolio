  ...
  return (
    ...
    >
      <div className="relative h-48 bg-gradient-to-br from-zinc-600 to-zinc-800">
        {(item as any).clientPhoto?.url ? (
          <ResponsiveImage
            src={(item as any).clientPhoto.url}
            alt={(item as any).clientPhoto.alt || item.title}
            fill
            className="object-cover group-hover:scale-105 transition-transform duration-300"
            variations={(item as any).clientPhoto.variations}
            targetWidth={400}
          />
        ) : (
      ...
      <div className="p-6">
          {(item as any).clientTitle && (
            <p className="text-sm text-zinc-700 font-medium mb-2">{(item as any).clientTitle}</p>
          )}
